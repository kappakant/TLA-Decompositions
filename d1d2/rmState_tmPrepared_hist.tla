...
NumRandElems == 9
TypeOKRand ==
/\ (rmState \in RandomSubset(10, [RMs -> {"working","prepared","committed","aborted"}]))
\*/\ (tmPrepared \in SUBSET(RMs))
/\ (tmPrepared \in RandomSubset(8, SUBSET(RMs)))
...
Inv19_1_0_def == \A rm \in RMs : (rmState[rm] = "prepared") \/ ((rmState[rm] = "working"))
Inv2_1_1_def == \A rm \in RMs : (rmState[rm] = "aborted") \/ ((rmState[rm] = "working"))
Inv1_1_0_def == \A rm \in RMs : (rmState[rm] = "aborted") \/ ((rmState[rm] = "prepared"))
